...
RSpec.describe Post, type: :model do
  ...
  context 'Scopes' do
    it 'default_scope orders by descending created_at' do
      ...
      expect(described_class.all).to eq [second_post, first_post]
    end
    ...
    it 'by_category scope gets posts by particular category' do
      ...
      create_list(:post, 10)
      posts = described_class.by_category(category.branch, category.name)
    ...
    it 'by_branch scope gets posts by particular branch' do
      ...
      create(:post, category_id: category.id)
      create_list(:post, 10)
      posts = described_class.by_branch(category.branch)
      expect(posts.count).to eq 1
      expect(posts[0].category.branch).to eq category.branch
    end

    it 'search finds a matching post' do
      post = create(:post, title: 'awesome title',
                           content: 'great content ' * 5)
      create_list(:post, 10, title: ('a'..('c' * 2)).to_a.shuffle.join)
      expect(described_class.search('awesome').count).to eq 1
      expect(described_class.search('awesome')[0].id).to eq post.id
      expect(described_class.search('great').count).to eq 1
      expect(described_class.search('great')[0].id).to eq post.id
    end
  end
end
